...
func (cq *ChatQuery) QueryAuthor() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, chat.AuthorTable, chat.AuthorColumn),
		)
...
func (cq *ChatQuery) WithAuthor(opts ...func(*UserQuery)) *ChatQuery {
	query := (&UserClient{config: cq.config}).Query()
...
func (cq *ChatQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Chat, error) {
	...
	if query := cq.withAuthor; query != nil {
		if err := cq.loadAuthor(ctx, query, nodes, nil,
			func(n *Chat, e *User) { n.Edges.Author = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ChatQuery) loadAuthor(ctx context.Context, query *UserQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Chat)
	for i := range nodes {
		if nodes[i].user_chats == nil {
			continue
		}
		fk := *nodes[i].user_chats
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_chats" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
