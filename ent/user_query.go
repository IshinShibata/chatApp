...
func (uq *UserQuery) loadChats(ctx context.Context, query *ChatQuery, nodes []*User, init func(*User), assign func(*User, *Chat)) error {
	...
	for _, n := range neighbors {
		fk := n.user_chats
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_chats" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_chats" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
